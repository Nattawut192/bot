...
token = "OTU4MTMxNDM4NzUzMDM4Mzk4.YkI3cA.8ZKQQ8LHwugS9lrajrZAkmFzE0E"
...
async def sms(ctx, phone, amount:int):
    ...
    if (amount < 151):
      ...
      embes = discord.Embed(title="SPAMSMS", description="Exsition.",color=0xff4612)
      embes.add_field(name="กำลังโจมตีไปยังเบอร์",value=phone)
...
@bot.command()
async def help(ctx):
	emBed = discord.Embed(title="PATTAYA BOT",description="วิธิใช้",color=0xff4612)
	emBed.add_field(name="Exsition.",value="!pty.sms [เบอร์] [จำนวน]")
